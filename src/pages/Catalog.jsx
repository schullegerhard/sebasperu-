...
// Sección "Sobre esta categoría" (mismos textos/imágenes del diseño).
const CATEGORY_INFO = {
  'laptops-pc': {
...
function CatProductCard({ p }) {
  ...
  const to = `/producto/${p.slug || productSlug(p.name)}`
...
export default function Catalog({ mode = 'category' }) {
  ...
  const base = useMemo(() => {
    ...
    if (mode === 'search') {
      ...
      const pool = [...storeProducts, ...catalogAll]
      const seen = new Set()
  ...
  useEffect(() => { setSelBrands([]); setSearchLocal(''); setPMin(minP); setPMax(maxP); setMinRating(0); setSort('destacados'); setSelAttrs({}) }, [mode, slug, query, minP, maxP])

  // Facetas de atributos "Filtro" (se conservan para categorías con atributos).
  const attrFacets = useMemo(() => {
    const facets = new Map()
    for (const p of base) for (const a of (p.attributes || [])) {
  ...
  const filtered = useMemo(() => {
    ...
    return r
  }, [base, selBrands, pMin, pMax, minRating, searchLocal, sort, activeAttrs])

  const title = mode === 'all' ? 'Todos los productos'
    : mode === 'offers' ? 'Ofertas'
      : mode === 'search' ? `Resultados para "${query}"`
        : meta?.title || catObj?.name || (base[0]?.categoryLabel) || 'Catálogo'
  const crumbs = [{ label: 'Inicio', to: '/' }, { label: title }]
  useSeo({ title, description: `Explora ${title} en SebasPeru. Filtra por marca, precio y disponibilidad.`, path: mode === 'category' ? `/categoria/${slug}` : mode === 'offers' ? '/ofertas' : mode === 'all' ? '/productos' : '/buscar', jsonLd: breadcrumbJsonLd(crumbs) })

  const info = mode === 'category' ? CATEGORY_INFO[slug] : null

  return (
    <div className="container page cat2">
      <div className="cat-crumb"><Link to="/">Inicio</Link><ChevronRight size={12} /><span>{title}</span></div>
            ...
            <div className="cat-fbox">
              ...
              {brands.map((b) => (
                <label className="cat-check" key={b}>
                  ...
                  <span>{b}</span><em>{base.filter((p) => p.brand === b).length}</em>
                </label>
              ))}
            </div>
          )}
